...
def offset_list_cos(co_list, offset):
    for c in range(len(co_list)):
        co_list[c] = round_up_array([ co_list[c][0]+offset[0], co_list[c][1]+offset[1] ])
    return
...
class UIWindow:
    ...
    def draw(self):
        ...
        if self.status != '':
            blf.position(self.status_font, self.status_pos[0], self.status_pos[1], 0)
            blf.color(self.status_font, self.status_color[0],self.status_color[1],self.status_color[2],self.status_color[3])
    ...
    def place_status_text(self,):
        text_width, text_height = calc_text_size(self.status, self.status_size, self.status_font)
    ...
    def add_rot_gizmo(self, mat, size, axis, thickness):
        gizmo_cont = UIGizmoContainer(self.shader_3d, len(self.gizmo_sets), mat, size, axis)

        if axis[0]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 0, 'ROT_X', [0.8,0.0,0.0,0.35], thickness)
            gizmo_cont.gizmos.append(giz)
        if axis[1]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 1, 'ROT_Y', [0.0,0.8,0.0,0.35], thickness)
            gizmo_cont.gizmos.append(giz)
        if axis[2]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 2, 'ROT_Z', [0.0,0.0,0.8,0.35], thickness)
            gizmo_cont.gizmos.append(giz)
    ...
    def add_scale_gizmo(self, mat, size, axis, thickness):
        gizmo_cont = UIGizmoContainer(self.shader_3d, len(gizmo_cont.gizmos), mat, size, axis)

        if axis[0]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 0, [0.8,0.0,0.0,0.5], thickness)
            gizmo_cont.gizmos.append(giz)
        if axis[1]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 1, [0.0,0.8,0.0,0.5], thickness)
            gizmo_cont.gizmos.append(giz)
        if axis[2]:
            giz = UIRotateGizmo(self.shader_3d, len(gizmo_cont.gizmos), size, 1.0, 2, [0.0,0.0,0.8,0.5], thickness)
            gizmo_cont.gizmos.append(giz)
        self.gizmo_sets.append(gizmo_cont)
    ...
    def resize_panel(self, mouse_co):
        ...
        offset = round_array( [ mouse_co[0]-self.panel_resize_cache[1][0], mouse_co[1]-self.panel_resize_cache[1][1] ] )
        ...
        if abs(offset[0]) > 0 and size_check:
            ...
            if panel.bounding_corner[0] + panel.width + offset[0] > bpy.context.region.width:
                offset[0] = bpy.context.region.width - panel.bounding_corner[0] - panel.width
            ...
            if self.panel_resize_cache[2]:
                ...
                for subp in panel.subpanels:
                    ...
                    for row in subp.rows:
                        ...
                        if len(row.items) > 0:
                            ...
                            width_tot -= (len(row.items)-1) * row.item_separation
                ...
                if 'C' in panel.alignment:
                    panel.position[0] = round(panel.bounding_corner[0] + panel.width/2)
    ...
    def move_panel(self, mouse_co):
        ...
        offset = round_array( [ mouse_co[0]-self.panel_move_cache[1][0], mouse_co[1]-self.panel_move_cache[1][1] ] )
        ...
        if abs(offset[0]) > 0 or abs(offset[1]) > 0:
            ...
            if panel.bounding_corner[1] + offset[1] > bpy.context.region.height:
                offset[1] = bpy.context.region.height - panel.bounding_corner[1]
                            ...
                            for item in row.items:
                                if item.type == 'NUMBER':
                                    if item.hover:
                                        ...
                                        item.value = round(item.value+fac, item.decimals)
                                        if item.value < item.min:
                                            item.value = item.min
                                        if item.value > item.max:
                                            item.value = item.max

                                        item.update_slider([panel.position[0]+subp.position[0]+row.position[0], panel.position[1]+subp.position[1]+row.position[1]])
                                        return item.value
        return None
                            ...
                            for item in row.items:
                                if item.type == 'NUMBER':
                                    if item.hover and item.bar_hover:
                                        ...
                                        self.num_slide_cache.append([panel.index, subp.index, row.index, item.index])
    ...
    def slide_number(self, mouse_co, shift, fac=0.01):
        ...
        offset = round_array( [ mouse_co[0]-self.num_slide_cache[1][0], mouse_co[1]-self.num_slide_cache[1][1] ] )
        if shift:
        ...
        if val_shift != 0.0:
            ...
            num.value = new_val
            num.update_slider([panel.position[0]+subp.position[0]+row.position[0], panel.position[1]+subp.position[1]+row.position[1]])
    ...
    def cancel_num_slide(self):
        num = self.panels[self.num_slide_cache[0][0]].subpanels[self.num_slide_cache[0][1]].rows[self.num_slide_cache[0][2]].items[self.num_slide_cache[0][3]]
                    ...
                    if subp.hover:
                        if subp.header_hover:
                            ...
                            if 'T' in panel.alignment:
                                subp.create_shape_data(y_pos, panel.horizontal_margin, panel.width, panel.alignment)
                            else:
                                subp.create_shape_data(subp.position[1]-subp.height, panel.horizontal_margin, panel.width, panel.alignment)
...
class UIBorder:
    ...
    def create_shape_data(self, text=None, color=None):
        ...
        [thick,thick], [thick,self.height-thick], [self.width-thick,self.height-thick], [self.width-thick,thick],
        ]
        ...
        if self.use_header:
            ...
            init_pos = [
            mathutils.Vector((center-size[0]-65,self.height-thick)), mathutils.Vector((center+size[0]+65,self.height-thick)), 
            mathutils.Vector((center+size[0]+10,self.height-thick-size[1]*1.5)), mathutils.Vector((center-size[0]-10,self.height-thick-size[1]*1.5)),
            ]
            ...
            if self.bevel_size > 0.0:
                mode_init_pos, mode_init_inds = bevel_ui(init_pos, [2,3], offset, self.bevel_size, self.bevel_res)
        ...
        self.batch = batch_for_shader(self.shader, 'TRIS', {"pos": self.points}, indices=self.tris)
        return
    ...
    def draw(self):
        ...
        if self.text != '':
            ...
            blf.color(self.font_id, self.text_color[0], self.text_color[1], self.text_color[2], self.text_color[3])
...
class UIPanel:
    ...
    def create_shape_data(self):
        ...
        self.header_render = fit_text(self.header_text, self.header_text_size, self.font_id, self.width-self.margin*2-self.text_margin-offset)
        text_width, text_height = calc_text_size(self.header_render, self.header_text_size, self.font_id)

        header_label_width = text_width
        if self.header_icon != None:
            header_label_width = text_width + offset

        self.header_text_x, self.header_text_y = place_text_in_box(self.header_align, self.header_bar_height, self.width-self.margin*2, self.text_margin, text_height, header_label_width, self.header_render, x_offset=offset)
        
        if self.header_icon != None:
            self.header_icon_pos.append([self.header_text_x-offset, -self.header_bar_height+self.icon_margin])
            self.header_icon_pos.append([self.header_text_x-offset+self.header_icon_size, -self.header_bar_height+self.icon_margin])
            self.header_icon_pos.append([self.header_text_x-offset+self.header_icon_size, -self.icon_margin])
            self.header_icon_pos.append([self.header_text_x-offset, -self.icon_margin])
        ...
        for subp in self.subpanels:
            subp.create_shape_data(self.panel_y_pos, self.horizontal_margin, self.width, self.alignment)
        ...
        if 'T' in self.alignment:
            ...
            y_offset = 0
            header_y_offset = 0
            self.subp_corner_offset = round_array( [ 0, -self.header_bar_height-self.panel_separation-self.vert_margin*2 ] )
            self.subp_stacking_offset = -self.header_bar_height-self.panel_separation-self.vert_margin*2
        ...
        self.bounding_corner = round_array( [ self.position[0]+self.x_offset, self.position[1]+self.y_offset ] )
        self.header_corner = round_array( [ self.position[0]+self.x_offset, self.position[1]+header_y_offset ] )


        self.box_points, self.box_tris, self.box_lines = calc_box(0, 0, self.width, self.height, [], self.bevel_size, self.bevel_res)
        border_out_points, border_out_tris, border_out_lines = calc_box(-self.border_size, self.border_size, self.width+self.border_size*2, self.height+self.border_size*2, [], self.bevel_size, self.bevel_res)
        
        self.border_points, self.border_tris = tris_between_boxes(self.box_points, border_out_points)


        self.header_points, self.header_tris, self.header_lines = calc_box(0, 0, self.header_bar_width, self.header_bar_height, [], self.header_bevel_size, self.header_bevel_res)


        self.subp_box_points, self.subp_box_tris, self.subp_box_lines = calc_box(0, 0, self.width, self.height-self.margin-header_height-self.border_size*2, [], self.bevel_size, self.bevel_res)
        border_out_points, border_out_tris, border_out_lines = calc_box(-self.border_size, self.border_size, self.width+self.border_size*2, self.height-self.margin-header_height, [], self.bevel_size, self.bevel_res)
        
        self.subp_border_points, self.subp_border_tris = tris_between_boxes(self.subp_box_points, border_out_points)
    ...
    def update_position(self):
        ...
        box_points = draw_cos_offset(self.header_corner[0], self.header_corner[1], self.box_points)
        box_lines = draw_cos_offset(self.header_corner[0], self.header_corner[1], self.box_lines)
        border_points = draw_cos_offset(self.header_corner[0], self.header_corner[1], self.border_points)
        header_points = draw_cos_offset(header_bar_x, header_bar_y, self.header_points)
        header_lines = draw_cos_offset(header_bar_x, header_bar_y, self.header_lines)
        subp_box_points = draw_cos_offset(self.bounding_corner[0]+self.subp_corner_offset[0], self.bounding_corner[1]+self.subp_corner_offset[1], self.subp_box_points)
        subp_box_lines = draw_cos_offset(self.bounding_corner[0]+self.subp_corner_offset[0], self.bounding_corner[1]+self.subp_corner_offset[1], self.subp_box_lines)
        subp_border_points = draw_cos_offset(self.bounding_corner[0]+self.subp_corner_offset[0], self.bounding_corner[1]+self.subp_corner_offset[1], self.subp_border_points)

        if self.header_icon != None:
            icon_pos = draw_cos_offset(header_bar_x, header_bar_y, self.header_icon_pos)
            tex_cos = [[0, 0], [1, 0], [1, 1], [0, 1]]
            self.batch_icon = batch_for_shader(self.icon_shader, 'TRI_FAN',{"pos": icon_pos, "texCoord": tex_cos})
        ...
        self.batch_bg = batch_for_shader(self.shader, 'TRIS', {"pos": box_points}, indices=self.box_tris)
        self.batch_bg_lines = batch_for_shader(self.shader, 'LINES', {"pos": box_lines})
        self.batch_border = batch_for_shader(self.shader, 'TRIS', {"pos": border_points}, indices=self.border_tris)
        self.batch_subp_bg = batch_for_shader(self.shader, 'TRIS', {"pos": subp_box_points}, indices=self.subp_box_tris)
        self.batch_subp_bg_lines = batch_for_shader(self.shader, 'LINES', {"pos": subp_box_lines})
        self.batch_subp_border = batch_for_shader(self.shader, 'TRIS', {"pos": subp_border_points}, indices=self.subp_border_tris)
        self.batch_header = batch_for_shader(self.shader, 'TRIS', {"pos": header_points}, indices=self.header_tris)
        self.batch_header_lines = batch_for_shader(self.shader, 'LINES', {"pos": header_lines})


        if self.use_visible_hov_icon:
            vis_hov_bg_points = draw_cos_offset(self.header_corner[0], self.header_corner[1], self.visible_icon_bg_points)
            vis_hov_points = draw_cos_offset(self.header_corner[0], self.header_corner[1], self.visible_icon_points)
            self.batch_visible_hov_icon_bg = batch_for_shader(self.shader, 'TRIS', {"pos": vis_hov_bg_points}, indices=self.visible_icon_bg_tris)
            self.batch_visible_hov_icon = batch_for_shader(self.shader, 'TRIS', {"pos": vis_hov_points}, indices=self.visible_icon_tris)
    ...
    def draw(self):
        if self.visible == True:
            ...
            if self.header_text != '':
                header_pos = [ self.position[0]+self.header_offset[0]+self.header_bar_offset[0], self.position[1]+self.header_offset[1]+self.header_bar_offset[1] ]
                blf.position(self.font_id, header_pos[0]+self.header_text_x, header_pos[1]+self.header_text_y, 0)
                blf.color(self.font_id, self.color_header_text[0],self.color_header_text[1],self.color_header_text[2],self.color_header_text[3])
    ...
    def add_visible_oh_hov_icon(self, size):
        self.visible_icon_bg_points = [[0,-size], [0,0], [size,0], [size,-size]]
        self.visible_icon_bg_tris = [[0,1,2], [0,2,3]]
        self.visible_icon_points = [ [2,-size+2], [2,-2], [size-2,-2], [size-7,-7], [7,-7], [7,-size+7] ]
    ...
    def test_hover(self, mouse_co):
        hov_item = None
        self.edge_hover = ui_hover_test(mouse_co, [ self.bounding_corner[0]-self.border_size*2, self.bounding_corner[1]], self.width+self.border_size*4, self.height)
        
        if self.edge_hover:
            self.hover = ui_hover_test(mouse_co, self.bounding_corner, self.width, self.height)
            if self.hover:
                self.header_hover = ui_hover_test(mouse_co, [self.position[0]+self.header_offset[0]+self.header_bar_offset[0],self.position[1]+self.header_offset[1]+self.header_bar_offset[1]], self.header_bar_width, self.header_bar_height)
                if self.header_hover:
                    ...
                    for subp in self.subpanels:
                        subp.hover = ui_hover_test(mouse_co, [self.position[0]+subp.position[0],self.position[1]+subp.position[1]], subp.width, subp.height)
                        if subp.hover:
                            if subp.use_header:
                                subp.header_hover = ui_hover_test(mouse_co, [self.position[0]+subp.position[0]+subp.header_bar_offset[0],self.position[1]+subp.position[1]+subp.header_bar_offset[1]], subp.header_bar_width, subp.header_bar_height)
                            else:
                                subp.header_hover = False
                            ...
                            if subp.header_hover == False:
                                ...
                                for row in subp.rows:
                                    for item in row.items:
                                        item.hover = ui_hover_test(mouse_co, [ self.position[0]+subp.position[0]+row.position[0]+item.position[0], self.position[1]+subp.position[1]+row.position[1]+item.position[1] ], item.width, item.height)
                                        ...
                                        if item.hover:
                                            ...
                                            if item.type == 'BUTTON':
                                                ...
                                                item.bar_hover = ui_hover_test(mouse_co, [ self.position[0]+subp.position[0]+row.position[0]+item.position[0]+item.box_arrow_size, self.position[1]+subp.position[1]+row.position[1]+item.position[1] ], item.width-item.box_arrow_size*2, item.height)
                                                item.left_hover = ui_hover_test(mouse_co, [ self.position[0]+subp.position[0]+row.position[0]+item.position[0], self.position[1]+subp.position[1]+row.position[1]+item.position[1] ], item.box_arrow_size, item.height)
                                                item.right_hover = ui_hover_test(mouse_co, [ self.position[0]+subp.position[0]+row.position[0]+item.position[0]+item.width-item.box_arrow_size, self.position[1]+subp.position[1]+row.position[1]+item.position[1] ], item.box_arrow_size, item.height)
...
class UISubpanel:
    ...
    def create_shape_data(self, y_pos, margin, p_width, align):
        ...
        if self.use_header:
            ...
            arrow_gap = round((self.header_bar_height - self.header_arrow_size)/2)
            self.arrow_right_cos = [ [self.margin, -arrow_gap], [self.margin+self.header_arrow_size, -arrow_gap-round(self.header_arrow_size/2)], [self.margin, -self.header_bar_height+arrow_gap] ]
            self.arrow_dwn_cos = [ [self.margin, -arrow_gap], [self.margin+self.header_arrow_size, -arrow_gap], [self.margin+round(self.header_arrow_size/2), -self.header_bar_height+arrow_gap] ]
            ...
            #CHECK TEXT FIT HEADER BAR
            self.header_render = fit_text(self.header_text, self.header_text_size, self.font_id, self.header_bar_width-arrow_width-self.text_margin)
            text_width, text_height = calc_text_size(self.header_render, self.header_text_size, self.font_id)


            #FIND TEXT POSITION INSIDE HEADER
            self.header_text_x, self.header_text_y = place_text_in_box(self.header_align, self.header_bar_height, self.width-self.margin*3-self.header_arrow_size, self.text_margin, text_height, text_width, self.header_render, x_offset = self.header_arrow_size)
    ...
    def update_position(self, p_pos):
        ...
        header_points = draw_cos_offset(x_co+self.header_bar_offset[0], y_co+self.header_bar_offset[1], self.header_points)
        header_lines = draw_cos_offset(x_co+self.header_bar_offset[0], y_co+self.header_bar_offset[1], self.header_lines)

        self.batch_bg = batch_for_shader(self.shader, 'TRIS', {"pos": box_points}, indices=self.box_tris)
        self.batch_header = batch_for_shader(self.shader, 'TRIS', {"pos": header_points}, indices=self.header_tris)
        self.batch_header_lines = batch_for_shader(self.shader, 'LINES', {"pos": header_lines})

        arrow_right_cos = draw_cos_offset(x_co+self.header_bar_offset[0], y_co+self.header_bar_offset[1], self.arrow_right_cos)
        arrow_dwn_cos = draw_cos_offset(x_co+self.header_bar_offset[0], y_co+self.header_bar_offset[1], self.arrow_dwn_cos)
        self.batch_arrow_right = batch_for_shader(self.shader, 'TRIS', {"pos": arrow_right_cos}, indices=[[0,1,2]])
        self.batch_arrow_down = batch_for_shader(self.shader, 'TRIS', {"pos": arrow_dwn_cos}, indices=[[0,1,2]])
    ...
    def draw(self, position):
        if self.visible:
            ...
            if self.use_header:
                ...
                if self.header_text != '':
                    blf.position(self.font_id, pos[0]+self.header_text_x+self.header_bar_offset[0], pos[1]+self.header_text_y+self.header_bar_offset[1], 0)
                    blf.color(self.font_id, self.color_header_text[0],self.color_header_text[1],self.color_header_text[2],self.color_header_text[3])
    ...
    def add_row(self):
        row = UIRow(self.shader, self.icon_shader, len(self.rows), self.width-self.margin*2, self.row_height, [self.position[0]+self.margin, self.position[1]-self.panel_y_pos-self.row_separation])
...
class UIRow:
    ...
    def add_button(self, id, text):
        ...
        but = UIButton(self.shader, self.icon_shader, len(self.items), id, text, width, self.height-self.margin*2, [self.margin, -self.margin])
    ...
    def add_label(self, text):
        ...
        label = UILabel(self.shader, self.icon_shader, len(self.items), text, width, self.height-self.margin*2, [self.margin, -self.margin])
    ...
    def add_bool_prop(self, id, text, box_size, default):
        ...
        boolean = UIBoolProp(self.shader, self.icon_shader, len(self.items), id, text, box_size, default, width, self.height-self.margin*2, [self.margin, -self.margin])
    ...
    def add_num_prop(self, id, text, num, decimals, factor, min_val, max_val):
        ...
        num = UINumProp(self.shader, self.icon_shader, len(self.items), id, text, num, int(decimals), factor, min_val, max_val, width, self.height-self.margin*2, [self.margin, -self.margin])
...
class UILabel:
    ...
    def create_shape_data(self, y_pos, margin):
        ...
        #CHECK TEXT FIT
        self.text_render = fit_text(self.text, self.font_size, self.font_id, self.width-self.text_margin-offset)
        text_width, text_height = calc_text_size(self.text_render, self.font_size, self.font_id)

        label_width = text_width
        if self.icon != None:
            label_width = text_width + offset
        
        self.text_x, self.text_y = place_text_in_box(self.text_align, self.height, self.width, self.text_margin, text_height, label_width, self.text_render, x_offset=offset)


        if self.icon != None:
            self.icon_pos.append([self.text_x-offset, -self.height+self.icon_margin])
            self.icon_pos.append([self.text_x-offset+self.icon_size, -self.height+self.icon_margin])
            self.icon_pos.append([self.text_x-offset+self.icon_size, -self.icon_margin])
    ...
    def draw(self, position):
        if self.visible == True:
            ...
            pos = [position[0]+self.position[0], position[1]+self.position[1]]
            if self.text_render != '':
                blf.position(self.font_id, pos[0]+self.text_x, pos[1]+self.text_y, 0)
                blf.color(self.font_id, self.color_text[0],self.color_text[1],self.color_text[2],self.color_text[3])
...
class UINumProp:
    ...
    def create_shape_data(self, y_pos, margin):
        ...
        if self.height*3 < self.width:
            ...
            self.perc_points, self.perc_tris, self.perc_lines = calc_box(0, 0, (self.width-self.box_arrow_size*2)*perc, self.height, [], 0, 0)
        else:
            self.box_arrow_size = round(self.width/3)
            self.box_points, self.box_tris, self.box_lines = calc_box(0, 0, self.box_arrow_size, self.height, [], 0, 0)
            self.perc_points, self.perc_tris, self.perc_lines = calc_box(0, 0, self.box_arrow_size*perc, self.height, [], 0, 0)


        #CHECK TEXT FIT HEADER BAR
        self.text_render = fit_text(self.text, self.font_size, self.font_id, self.width-self.box_arrow_size*2-self.text_margin, value=str(self.value))
        text_width, text_height = calc_text_size(self.text_render, self.font_size, self.font_id)
        self.text_x, self.text_y = place_text_in_box(self.text_align, self.height, self.width-self.box_arrow_size*2, self.text_margin, text_height, text_width, self.text_render, x_offset=self.box_arrow_size)



        self.box_right_points, self.box_right_tris, self.box_right_lines = calc_box(0, 0, self.box_arrow_size, self.height, [1,2], self.bevel_size, self.bevel_res)
        self.box_left_points, self.box_left_tris, self.box_left_lines = calc_box(0, 0, self.box_arrow_size, self.height, [0,3], self.bevel_size, self.bevel_res)
        ...
        if self.box_arrow_size > self.arrow_width:
            ...
            self.arrow_left_lines = [ [self.box_arrow_size-x_gap,-y_gap], [x_gap,round(-self.height/2)], [self.box_arrow_size-x_gap,-self.height+y_gap], ]
            self.arrow_right_lines = [ [x_gap,-y_gap], [self.box_arrow_size-x_gap,round(-self.height/2)], [x_gap,-self.height+y_gap], ]
    ...
    def update_position(self, p_pos):
        ...
        box_points = draw_cos_offset(x_co+self.box_arrow_size, y_co, self.box_points)
        box_lines = draw_cos_offset(x_co+self.box_arrow_size, y_co, self.box_lines)
        perc_points = draw_cos_offset(x_co+self.box_arrow_size, y_co, self.perc_points)
        perc_lines = draw_cos_offset(x_co+self.box_arrow_size, y_co, self.perc_lines)

        bool_left_points = draw_cos_offset(x_co, y_co, self.box_left_points)
        bool_right_points = draw_cos_offset(x_co+self.width-self.box_arrow_size, y_co, self.box_right_points)
        bool_left_lines = draw_cos_offset(x_co, y_co, self.box_left_lines)
        bool_right_lines = draw_cos_offset(x_co+self.width-self.box_arrow_size, y_co, self.box_right_lines)

        arrow_left_lines = draw_cos_offset(x_co, y_co, self.arrow_left_lines)
        arrow_right_lines = draw_cos_offset(x_co+self.width-self.box_arrow_size, y_co, self.arrow_right_lines)


        self.batch = batch_for_shader(self.shader, 'TRIS', {"pos": box_points}, indices=self.box_tris)
        self.batch_perc = batch_for_shader(self.shader, 'TRIS', {"pos": perc_points}, indices=self.perc_tris)

        self.batch_bool_left = batch_for_shader(self.shader, 'TRIS', {"pos": bool_left_points}, indices=self.box_left_tris)
        self.batch_bool_right = batch_for_shader(self.shader, 'TRIS', {"pos": bool_right_points}, indices=self.box_right_tris)
        ...
        self.batch_arrow_left = batch_for_shader(self.shader, 'TRIS', {"pos": arrow_left_lines}, indices=[[0,1,2]])
        self.batch_arrow_right = batch_for_shader(self.shader, 'TRIS', {"pos": arrow_right_lines}, indices=[[0,1,2]])
...
class UIBoolProp:
    def __init__(self, shader, icon_shader, index, id, text, box_size, value, width, height, position):
        ...
        self.position_bool = round_array([ (self.height-self.box_bool_size)/2, ((self.height-self.box_bool_size)/2)*-1  ])
    ...
    def create_shape_data(self, y_pos, margin):
        ...
        #CHECK TEXT FIT HEADER BAR
        self.text_render = fit_text(self.text, self.font_size, self.font_id, self.width-bool_marg-self.box_bool_size-self.text_margin)
        text_width, text_height = calc_text_size(self.text_render, self.font_size, self.font_id)
        self.text_x, self.text_y = place_text_in_box(self.text_align, self.height, self.width, self.text_margin, text_height, text_width+bool_marg+self.box_bool_size, self.text_render, x_offset=bool_marg+self.box_bool_size)

        self.position_bool = round_array([self.text_x-bool_marg-self.box_bool_size, -( (self.height-self.box_bool_size)/2 )])

        self.box_points, self.box_tris, self.box_lines = calc_box(0, 0, self.width, self.height, [], self.bevel_size, self.bevel_res)
        self.box_bool_points, self.box_bool_tris, self.box_bool_lines = calc_box(0, 0, self.box_bool_size, self.box_bool_size, [], 0, 0)
        self.bool_lines = [ [2,-2], [self.box_bool_size-2,-self.box_bool_size+2], [2,-self.box_bool_size+2], [self.box_bool_size-2,-2] ]
    ...
    def update_position(self, p_pos):
        ...
        box_points = draw_cos_offset(x_co, y_co, self.box_points)
        box_lines = draw_cos_offset(x_co, y_co, self.box_lines)
        box_bool_points = draw_cos_offset(x_bool_co, y_bool_co, self.box_bool_points)
        box_bool_lines = draw_cos_offset(x_bool_co, y_bool_co, self.box_bool_lines)
        bool_lines = draw_cos_offset(x_bool_co, y_bool_co, self.bool_lines)

        self.batch = batch_for_shader(self.shader, 'TRIS', {"pos": box_points}, indices=self.box_tris)

        if self.box_bool_size > self.width:
            self.batch_bool_box = batch_for_shader(self.shader, 'TRIS', {"pos": []}, indices=[])
            self.batch_bool = batch_for_shader(self.shader, 'LINES', {"pos": []})
        else:
            self.batch_bool_box = batch_for_shader(self.shader, 'TRIS', {"pos": box_bool_points}, indices=self.box_bool_tris)
            self.batch_bool = batch_for_shader(self.shader, 'LINES', {"pos": bool_lines})
...
class UIButton:
    ...
    def create_shape_data(self, y_pos, margin):
        ...
        #CHECK TEXT FIT HEADER BAR
        self.text_render = fit_text(self.text, self.font_size, self.font_id, self.width-self.text_margin)
        text_width, text_height = calc_text_size(self.text_render, self.font_size, self.font_id)
        self.text_x, self.text_y = place_text_in_box(self.text_align, self.height, self.width, self.text_margin, text_height, text_width, self.text_render)
...
class UIGizmoContainer:
    ...
    def update_position(self, matrix, ang=0):
        ...
        self.scale_factor = None
        for gizmo in self.gizmos:
            self.scale_factor = gizmo.update_position(matrix, self.scale_factor, ang)
        return
    ...
    def update_rot(self, ang, start_ang):
        for gizmo in self.gizmos:
            if gizmo.in_use:
                gizmo.update_rot_fan(self.matrix, self.scale_factor, ang, start_ang)
        return
...
class UIRotateGizmo:
    ...
    def create_shape_data(self):
        ...
        for i in range(self.resolution):
            ...
            if i < self.resolution-1:
                self.tris += [
                    [i*4+1, i*4, i*4+4], [i*4+1, i*4+4, i*4+5], [i*4+2, i*4+1, i*4+5], [i*4+2, i*4+5, i*4+6],
                    [i*4+3, i*4+2, i*4+6], [i*4+3, i*4+6, i*4+7], [i*4, i*4+3, i*4+7], [i*4, i*4+7, i*4+4]
                ]
            else:
                self.tris += [
                    [i*4+1, i*4, 0], [i*4+1, 0, 1], [i*4+2, i*4+1, 1], [i*4+2, 1, 2],
    ...
    def update_position(self, matrix, scale_fac, angle=0):
        ...
        self.batch = batch_for_shader(self.shader, 'TRIS', {"pos": self.mat_points}, indices=self.tris)
    ...
    def update_rot_fan(self, matrix, scale_fac, angle, start_ang=0):
        ...
        for i in range(point_num+1):
            ...
            if self.axis == 0:
                eul = mathutils.Euler((math.radians(90.0), 0.0, math.radians(90.0)), 'XYZ')
                new_co.rotate(eul)
                self.fan_points.append(new_co)
            if self.axis == 1:
        ...
        self.batch_fan = batch_for_shader(self.shader, 'TRIS', {"pos": self.mat_fan_points}, indices=self.fan_tris)
        self.batch_fan_lines = batch_for_shader(self.shader, 'LINES', {"pos": self.mat_fan_lines})
    ...
    def draw(self):
        if self.active:
            if self.in_use:
                ...
                self.shader.bind()
                self.shader.uniform_float("color", [self.color[0],self.color[1],self.color[2],0.2])
