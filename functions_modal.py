    ...
    if avg_vec.length > 0.0:
        for ind in po_inds:
            ...
            if po.valid:
                for l in range(len(po.loop_normals)):
                    loop_norm_set(
                        self, po, l, po.loop_normals[l], avg_vec.copy())
        set_new_normals(self)
        add_to_undostack(self, 1)
        ...
        for ind in po_inds:
            ...
            for l, l_norm in enumerate(loop_norms):
                ...
                if cnt > 0:
                    ...
                    loop_norm_set(self, po, l, po.loop_normals[l], po.loop_normals[l].lerp(
                        smooth_vec, abn_props.smooth_strength))
                    del(smooth_vec)
        ...
        if po.valid:
            if self._object_smooth:
                ...
                if poly_norm.length > 0.0:
                    for l, l_ind in enumerate(po.loop_inds):
                        loop_norm_set(
                            self, po, l, po.loop_normals[l], poly_norm/len(po.loop_inds))

            else:
                for i, l_ind in enumerate(po.loop_inds):
                    for loop in self._object_bm.verts[po.index].link_loops:
                        if loop.index == l_ind:
                            po.loop_normals[i] = self._object.data.polygons[loop.face.index].normal.copy(
                            ) * direction
        ...
        for ind in po_inds:
            if ind != self._active_point:
                ...
                if po.valid:
                    ...
                    for l in range(len(po.loop_normals)):
                        loop_norm_set(
                            self, po, l, po.loop_normals[l], self._points_container.points[self._active_point].loop_normals[inds[l]].copy())
    ...
    if self._copy_normals != None and self._copy_normals_vecs != None and self._copy_normal_ind != None:
        for ind in po_inds:
            ...
            if po.valid:
                ...
                for l in range(len(po.loop_normals)):
                    loop_norm_set(
                        self, po, l, po.loop_normals[l], self._copy_normals[inds[l]].copy())
...
def rotate_vectors(self, angle):
    ...
    for p, p_ind in enumerate(sel_pos):
        ...
        for l, l_norm in enumerate(po.loop_normals):
            ...
            loop_norm_set(self, po, l, og_norm, ((self._object.matrix_world.inverted(
...
def start_sphereize_mode(self):
    ...
    panel = self._window.add_panel(
        header_text='Sphereize Normals', hover_highlight=True)
    panel.width = 225
    ...
    row = subp.add_row()
    row.add_num_prop(58, 'Sphereize Strength',
                     self.target_strength, 2, 0.01, 0.01, 1.0)
    ...
    for i, ind in enumerate(po_inds):
        ...
        if po.valid:
            vec = (self._object.matrix_world.inverted() @ po.co) - \
                (self._object.matrix_world.inverted() @ self.target_emp.location)
            for l, l_ind in enumerate(po.loop_inds):
                loop_norm_set(
                    self, po, l, self._changing_po_cache[-1][i][l], self._changing_po_cache[-1][i][l].lerp(vec, self.target_strength))
...
def start_point_mode(self):
    ...
    self._window.panels[0].visible = False
    panel = self._window.add_panel(
        header_text='Point Normals at Target', hover_highlight=True)
    panel.width = 225
    panel.position = [self._mouse_loc[0], self._mouse_loc[1]]
    panel.alignment = 'TL'
    subp = panel.add_subpanel(self._window.scale, header_text='')
    ...
    row = subp.add_row()
    row.add_num_prop(58, 'Point Strength',
                     self.target_strength, 2, 0.01, 0.01, 1.0)
...
def point_normals(self, po_inds):
    if self.point_align:
        sel_cos = self._points_container.get_selected_cos()
        avg_loc = average_vecs(sel_cos)
        vec = (self._object.matrix_world.inverted() @ self.target_emp.location) - \
    ...
    for i, ind in enumerate(po_inds):
        ...
        if po.valid:
            if self.point_align == False:
                ...
                ) @ self.target_emp.location) - (self._object.matrix_world.inverted() @ po.co)
            for l, l_ind in enumerate(po.loop_inds):
...
def move_target(self, shift):
    offset = [self._mouse_loc[0] - self._changing_po_cache[0]
              [0], self._mouse_loc[1] - self._changing_po_cache[0][1]]
    ...
    new_co = view3d_utils.region_2d_to_location_3d(
        region, rv3d, self._changing_po_cache[3], self._changing_po_cache[1])
    if self.translate_mode == 0:
...
def relocate_gizmo_panel(self):
    ...
    if rco != None:
        self._window.reposition_panel(
            2, [rco[0]+panel.reposition_offset[0], rco[1]+panel.reposition_offset[1]])
    return
...
def button_pressed(self, event, but_type, but_id):
    ...
    # CANCEL NORMAL CHANGES
        ...
        if len(sel_pos) != 0:
            ...
            if but_id == 20:
                smooth_selected_normals(
                    self, sel_pos, abn_props.smooth_iters, 0.5)
...
def cache_point_data(self):
    ...
    for v in self._object_bm.verts:
        if len(v.link_loops) > 0:
            ...
            self._points_container.add_point(
                v.co, v.normal, loop_norms, loop_inds)
        else:
            self._points_container.add_empty_point(
            ...
            if rco != None:
                ...
                if nearest_point == None or dist < nearest_dist:
                    ...
                    if dist < radius:
                        ...
                        valid_po = False
                        if bvh != None:
                            hit = ray_cast_view_occlude_test(
                                point.co, mouse_co, bvh)
...
def lasso_selection_test(self, context, event):
    ...
    in_range_inds = bounding_box_filter(self._changing_po_cache, [view3d_utils.location_3d_to_region_2d(
        region, rv3d, po.co) for po in self._points_container.points])
        ...
        if point.valid and point.hide == False:
            ...
            if rco != None:
                if bvh != None:
                    ...
                    if hit == False:
                        sel_res = test_point_in_shape(
                            self._changing_po_cache, rco)
...
def keymap_initialize(self):
    ...
    row = subp.add_text_row(
        'Ctrl + Shift + X - Redo Selection Change', txt_size)
    return
...
def keymap_refresh_lasso(self):
    ...
    row = subp.add_text_row(
        'L-Ctrl + Shift Release - Remove from Selection', txt_size)
